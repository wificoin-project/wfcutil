...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := wfcutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(wfcutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := wfcutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := wfcutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
