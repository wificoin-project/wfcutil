...
func (cs *CoinSet) TotalValue() (value wfcutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue wfcutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue wfcutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount wfcutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue wfcutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue wfcutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue wfcutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue wfcutil.Amount, coins []Coin) (Coins, error) {
...
// wfcutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() wfcutil.Amount {
	return wfcutil.Amount(c.txOut().Value)
}
